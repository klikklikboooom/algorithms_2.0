...
vector<int> asyncShortestPath(vector<vector<neighbors>> adjacencyList, int destinationPath) {
    vector<int> memo;
    ...
    memo[destinationPath] = 0;


    queue<int> activeNodes;
    activeNodes.push(destinationPath);

    while(!activeNodes.empty()) {
        int thisActiveNode = activeNodes.front();
        vector<neighbors> outgoingNodes = findOutgoingEdgesNodes(thisActiveNode, adjacencyList);
        for(int k =0; k<outgoingNodes.size();k++) {
            ...
            if(memo[thisActiveNode] != INT_MAX && memo[outgoingNodes[k].node] > memo[thisActiveNode] + outgoingNodes[k].weight) {
                memo[outgoingNodes[k].node] = memo[thisActiveNode] + outgoingNodes[k].weight;
                successor[outgoingNodes[k].node] = thisActiveNode;
                activeNodes.push(outgoingNodes[k].node);
            }
        }
        activeNodes.pop(); 
    }
...
int main() {
    ...
    vector<int> shortestPathToDest = asyncShortestPath(adjacencyList, 5);
    for(int i =0;i<shortestPathToDest.size();i++)
        cout<<shortestPathToDest[i]<<endl;
    return 0;
}